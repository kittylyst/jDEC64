...
class Base64Test {
    ...
    void simpleAdd() {
        ...
        @DEC64
        long ten = of(10, (byte) 0);
        @DEC64
        long ten2 = of(1, (byte) 1);
    ...
    void simple_reciprocal() {
        @DEC64
        long half = reciprocal(DEC64_TWO);
        assertTrue(
    ...
    void simpleDivide() {
        ...
        @DEC64
        long three_p_five = of(35, (byte) -1);
        assertTrue(equals64(divide(SEVEN, DEC64_TWO), three_p_five), "7 / 2 should equal 3.5");
        @DEC64
        long zero_p_one_two_five = divide(DEC64_ONE, EIGHT);
        assertTrue(
        ...
        @DEC64
        long third = divide(DEC64_ONE, THREE);
    ...
    void complex_divide() {
        // 5.191 / 3.0955
        @DEC64
        long fiveAndABit = of(5_191L, (byte) -3);
        @DEC64
        long threeAndABit = of(3_095_5L, (byte) -4);
        @DEC64
        long expected = of(1_676_950_411_88L, (byte) -11);

        @DEC64
        long actual = divide(fiveAndABit, threeAndABit);
    ...
    void genEquals() {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            exp--;
            @DEC64
            long check = last + exponentAsLong(exp);
            assertTrue(
            ...
            @DEC64
            long canonical = canonical(check);
            assertTrue(
                    DEC64_ONE == canonical,
                    canonical
                    + "("
    ...
    void testIncDec() {
        ...
        for (byte exp = 1; exp < Byte.MAX_VALUE; exp++) {
            @DEC64
            long large = of(DEC64_MAX_COEFFICIENT, exp);
    ...
    void complexFormatSTD() {
        ...
        assertEquals("1.0", s);
        @DEC64
        long largerOne = 256_000_000_000_000_241L;

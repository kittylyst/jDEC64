...
public final class Basic64 {
     ...
     * (roundDir == -1) or greater than or equal to 'number' (roundDir == 1). In
     * the result, the exponent will be greater than or equal to zero unless it
     * is nan. Numbers with positive exponents will not be modified, even if the
    ...
    long largestInt(@DEC64 long number, int roundDir) {
        if (isNaN(number)) {
            return DEC64_NAN;
        }

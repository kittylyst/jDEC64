...
/**
 * A simple, non-resizing implementation of a stack that holds @DEC64 values. Zero-allocation,
 * except for exception throwing. Fully synchronized.
 *
 * @author kittylyst
 */
public final class StackDec64 {
   ...
   */
  public synchronized @DEC64 long peek() {
    int len = size();
   ...
   */
  public synchronized @DEC64 long push(@DEC64 long value) {
    int len = top++;
    contents[len] = value;
   ...
   * @throws EmptyStackException if this stack is empty.
   */
  public synchronized @DEC64 long pop() {
    @DEC64 long out = peek();
    top--;
  ...
  /**
   * Returns the 1-based position where a value is on this stack. If the value <tt>v</tt> occurs as
   ...
   * @return the 1-based position from the top of the stack where the value is located; the return
   *     value <code>-1</code> indicates that the value is not present on the stack.
   */
   ...
   *
   * @param v the desired value.
   * @return the index in the stack where the value is located; the return value <code>-1</code>
   *     indicates that the value is not present on the stack.
   */
  public synchronized int lastIndexOf(@DEC64 long v) {
    for (int i = top; i >= 0; i--) {
      if (contents[i] == v) return i;
    }
    return -1;
  }
}
